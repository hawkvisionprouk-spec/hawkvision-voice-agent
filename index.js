...
app.post('/voice', async (req, res) => {
  ...
  callSessions[callSid] = {
    ...
    messages: [
      {
        role: 'system',
        content: `You are SkyHawk, AI sales assistant for Hawk Vision Pro, a CCTV security shop in Sheffield UK.
RULES:
- Always respond in English only
- Keep responses SHORT — max 2 sentences
- Never ask more than one question at a time
- When customer wants WhatsApp, say "Sending now" and end with [SEND_WHATSAPP]
- When call exceeds 2 minutes or question is complex, say "Please visit our website at hawkvisionpro.co.uk, our AI assistant will help you" then end with [END_CALL]
...
- Website: hawkvisionpro.co.uk`
  ...
  twiml.say(
    { voice: 'Polly.Brian', language: 'en-GB' },
    'Hello, thank you for calling Hawk Vision Pro. I am SkyHawk, your AI assistant. How can I help you today?'
  );
  twiml.gather({
    input: 'speech',
...
app.post('/voice/respond', async (req, res) => {
  ...
  if (!session || !speechResult) {
    twiml.say({ voice: 'Polly.Brian' }, 'Sorry, I did not catch that. Please call again. Goodbye.');
    twiml.hangup();
    res.type('text/xml');
  ...
  if (elapsed > 120) {
    twiml.say(
      { voice: 'Polly.Brian' },
      'Please visit our website at hawk vision pro dot co dot uk. Our AI assistant will help you in detail. Goodbye!'
    );
    await sendWhatsApp(
      session.caller,
  ...
  const productKeywords = ['camera', 'cctv', 'dvr', 'nvr', 'poe', 'system', 'megapixel', 'outdoor', 'indoor', 'security'];
  if (productKeywords.some(k => speechResult.toLowerCase().includes(k))) {
    const products = await searchShopify(speechResult);
    if (products.length > 0) {
      session.messages.push({
        role: 'system',
        content: `Available products: ${products.map(p => `${p.title} £${p.price}`).join(', ')}`
      });
    }
  }
